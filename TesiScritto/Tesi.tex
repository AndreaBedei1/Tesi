...
\end{figure}
La struttura della pagine è molto semplice essa presenta un menù di navigazione comune alle pagine interne del sito. Esso permette di navigare il sito oppure di eseguire il logout.\\
La parte principale della pagina si compone da due sezioni la prima rappresenta una mappa dove sono segnati tutti gli avvistamneti attraverso dei marker. L'utente ha la possibilità di muoversi all'interno della sezione della mappa e in caso passaggio del mouse sui marker essi, attraverso dei popup mostreranno le informazioni principali sull'avvistamento. Se invece un marker viene cliccato viene aperta la pagina dell'avvistamento. Nella sezione di destra invece è presente una tabella in cui vengono aggiunti tutti gli avvistamenti con le informazioni più importanti. La tabella è stata realizzata rispettando tutte le direttive sull'accessibilità. Cliccando sul bottone vedi, si aprirà la pagina relativa all'avvistamento. Entrambe le sezioni in caso di hover cambiano aspetto per motivi stilistici. Infine, in alto sulla destra è presente un bottone che permette l'aggiunta di un nuovo avvistamento attraverso un modale che compare in sovraimpressione.
\begin{figure}[hbtp]
\centering
\includegraphics[scale=0.31]{img_concettuale/aggiungiAvv.png}
