...
Nelle specifiche di progetto è stata chiesta la possibilità di riconoscere in modo automatico la specie di delfino rappresentata delle immagini caricate degli operatori. Per fare questo si è utilizzato un algoritmo di visione artificiale il quale attraverso la segmentazione delle pinne degli individui permette di individuare di aiutare l'utente a individiduare la specie.
...
\section{Dataset}
Le balene e i delfini presenti nei dati a disposizione possono essere identificati attraverso le loro forme, le caratteristiche e le segnalazioni (alcune naturali, altre acquisite) delle loro pinne dorsali, dorsi, teste e fianchi. Alcune specie e alcuni individui hanno caratteristiche molto distinte, mentre altre sono molto meno distinte. Inoltre, le caratteristiche individuali possono variare nel tempo. Questa competizione amplia in modo significativo questo compito: i dati in gara contengono immagini di oltre 15.000 singoli mammiferi marini di 30 specie diverse, raccolte da 28 diverse organizzazioni di ricerca. Gli individui sono stati identificati manualmente e assegnati un ID-individuo dai ricercatori marini, e il compito è quello di identificare correttamente questi individui nelle immagini.
...
\toprule
\textbf{Specie} & \textbf{Numero di immagini per ogni animale.} \\
\midrule
\endfirsthead
%
\endhead
...
Da come si può notare dalla tabella per cisacuna specie c'è un diverso numero di immagini, quindi ci sarà un grado di accuratezza diverso per ciascuna categoria.

\section{Algoritmo di riconoscimento}
Per quanto riguarda l'algoritmo di riconoscimento è diviso in due parti:
\begin{itemize}
    \item Fase di training
    \item Fase di riconosciemnto dell'immagine
\end{itemize}
Entrambe le fasi sono realizzate attraverso degli script in codice python, le quali attraverso comandi bash sono state integrate nel codice php del server web.
...
Il codice che è stato utlizzato per realizzare questa funzionalità verrà scomposta in più parti in modo da descriverlo nel dettaglio.
...
\end{lstlisting}
La prima riga permette di verificare che siano stati caricati nel DataFrame tutti i dati del file, nella seconda si verifica che il numero di specie(nomi unici) sia corretto. Invece la terza permette di visuaizzare tutti i nomi delle specie col relativo numero di immagini ad esse associate.\\
...
\begin{itemize}
    ...
    \item \textbf{batch\_size}: specifica il numero di immagini da restituire in un singolo batch(sottoinsieme di dati) e viene impostato su 32 in questo caso. L'argomento shuffle viene impostato su True, il che significa che le immagini verranno mescolate casualmente durante l'iterazione del generatore.
...
\end{lstlisting}
Questo codice quasi identico a quello descritto sopra e crea un generatore di immagini di test per un modello di apprendimento automatico, settando i reletivi argomenti. 
...
Infine, viene aggiunto un ultimo strato denso con 30 unità e un'attivazione "softmax". L'attivazione "softmax" è un'attivazione che produce una distribuzione di probabilità delle classi. Questa attivazione è spesso utilizzata come ultimo strato di una rete neurale, poiché trasforma l'uscita della rete in una probabilità per ogni classe. In questo caso, l'attivazione softmax con 30 unità viene utilizzata come ultimo strato del modello per classificare le immagini in 30 categorie diverse.\\
...
\begin{itemize}
    ...
    \item \textbf{Time}: Tempo totale che ci vuole per completare un singolo step di allenamento. Gli step di allenamento sono il numero di passaggi che il modello compie sull'intero set di dati di addestramento prima di terminare un'epoca. Ogni step di addestramento è costituito da un gruppo di esempi di addestramento chiamati batch. Il numero di step di addestramento per un'epoca è calcolato dividendo il numero totale di esempi di addestramento per il numero di esempi in un singolo batch. Questo significa che, per ogni epoca, il modello si eserciterà su ogni esempio di addestramento una volta e solo una volta.
    ...
    \item \textbf{Training accuracy}: è una metrica che misura l'accuratezza della previsione del modello sui dati di allenamento. Viene calcolato come la percentuale di previsioni esatte rispetto al numero totale di previsioni effettuate dal modello sui dati di allenamento. Esso è utilizzato per valutare la performance del modello durante l'allenamento e per verificare che esso stia effettivamente imparando dai dati di allenamento. Tuttavia, esso non è un buon indicatore della performance del modello sui dati di test o sui dati nuovi, poiché c'è il rischio che il modello abbia imparato a memoria i dati di allenamento invece che generalizzare davvero le informazioni presenti in essi.
    \item \textbf{Validation loss}: è una metrica utilizzata per valutare la performance del modello durante la validazione. Questa metrica rappresenta la quantità di errore che il modello commette quando si prevede le classificazioni per i dati di validazione. Il valore di Validation loss viene calcolato come la media della funzione di costo tra le previsioni effettuate dal modello e le verità di etichetta per i dati di validazione. Più basso è il suo valore, migliore è la capacità del modello di generalizzare su dati nuovi.
...
\newpage
\subsubsection{Rappresentazione delle curve di perdiata}
...
il grafico che rappresenta la perdita ovvero la misura della discrepanza tra i valori predetti dal modello e i valori reali attraverso l'addestramento porta a minimizzare la perdita in modo tale che i valori predetti si avvicinino sempre di più a quelli reali. Invece, il grafivo dell'accuratezza ovvero la percentuale di predizioni corrette effettuate dal modello rispetto al numero totale di predizioni effettuate, con l'addestramento essa si alza.
...
Successivamente, il codice stampa a video l'accuratezza ottenuta sul test set, utilizzando.
...
In questa fase esamineremo il codice al fine di identificare una nuova immagine che passeremo all'algoritmo addestrato al fine di riconoscere la specie. Per come è stato implementato la rete neurale viene addestartata periodicamente in caso di aggiunta di nuove immaggini, invece ongi volta che si vuole utilizzare l'algoritmo bisogna caricare un file nel quale è presente l'addestramento della rete.
